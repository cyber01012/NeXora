...
type ModalType = null | "create-volunteer" | "change-password";
...
export function NGODashboard() {
  ...
  const handleCreateVolunteer = async (data: DynamicUserFormData) => {
    setLoading(true);
    try {
      const response = await authApi.createVolunteer({
      ...
      toast.error(getApiErrorMessage(error, "Failed to create volunteer."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-900 via-green-800 to-gray-900 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] rounded-lg p-8 backdrop-blur-sm">
          ...
          <p className="font-['Orbitron',sans-serif] text-[16px] text-[rgba(255,255,255,0.6)] mb-8">
            Manage volunteers and relief operations (requires NGO login)
          </p>

          <div className="flex gap-4">
            <button
              onClick={() => setActiveModal("create-volunteer")}
      ...
      {activeModal === "create-volunteer" && (
        <AuthModalCard title="Create Volunteer" onClose={() => setActiveModal(null)}>
          <DynamicUserForm
            submitButtonText="Create Volunteer"
            loading={loading}
