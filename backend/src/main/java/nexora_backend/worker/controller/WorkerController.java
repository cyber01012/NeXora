...
public class WorkerController {
    ...
    public ApiResponse<List<WorkerTaskResponse>> getTasks(
        ...
        if ("PENDING_ACCEPTANCE".equalsIgnoreCase(status) || "PENDING".equalsIgnoreCase(status)) {
            tasks = taskService.getPendingTasks(username);
        } else if ("IN_PROGRESS".equalsIgnoreCase(status) || "ACTIVE".equalsIgnoreCase(status)) {
            tasks = taskService.getActiveTasks(username);
        } else {
            // ALL / no filter: all active tasks assigned to this worker (no final decision)
