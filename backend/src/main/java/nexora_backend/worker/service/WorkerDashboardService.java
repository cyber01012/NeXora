...
public class WorkerDashboardService {
    ...
    public WorkerDashboardResponse getDashboard(String workerUsername) {
        ...
        // Resolve dept ID for scoped queries
        Long deptId = worker.isNull() ? null : worker.getDepartmentId();
        ...
        // Pending acceptance — use dept-scoped query when dept is known
        long pending;
        if (deptId != null) {
            pending = complaintRepository
        ...
        // Monthly completed — scoped to dept if possible
        List<ForwardedComplaint> allComplaints = deptId != null
                ? complaintRepository.findByWorker_UsernameCreatedAndDepartment_DeptIdOrderBySubmitDateDesc(workerUsername, deptId)
