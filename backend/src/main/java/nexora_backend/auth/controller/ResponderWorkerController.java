...
public class ResponderWorkerController {
    ...
    @PostMapping
    public ResponseEntity<RegistrationResponse> createWorker(
            @AuthenticationPrincipal AuthenticatedUser responderUser,
            @Valid @RequestBody VolunteerWorkerRegistrationRequest request
    ) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(authenticationFacade.registerWorker(request, responderUser));
    }
}
